                ...
                <form class="w-full" action="{{ route('book.create') }}" method="POST">
                    ...
                    <div>
                        <div class="px-4 sm:px-0">
                            <h3 class="text-base font-semibold leading-7 text-gray-900">本の新規登録</h3>
                        </div>
                        <div class="mt-6 border-t border-gray-100">
                            <dl class="divide-y divide-gray-100">
                            <div class="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
                                {{-- <dt class="text-sm font-medium leading-6 text-gray-900">ID</dt>
                                <dd class="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">自動で振り分けます</dd> --}}
                            ...
                            <div class="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
                                ...
                                <dd class="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
                                    ...
                                    <div class="mt-2">
                                        <input type="text" name="name" id="name" autocomplete="given-name" value="{{ old('name') }}" class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6">
                                        {{-- バリデーションのエラー表示 --}}
                                        @error('name')
                                    ...
                                    <div class="mt-2">
                                        <select id="status" name="status" autocomplete="country-name" class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:max-w-xs sm:text-sm sm:leading-6">
                                            ...
                                            @foreach(App\Models\Book::BOOK_STATUS_OBJECT as $key => $value)
                                                <option value="{{ $key }}" @if ((int)old('status')) selected @endif>{{ $value }}</option>
                                            @endforeach
                                            {{-- app/Models/Book.phpに定数を定義したやり方 --}}
                            ...
                            <div class="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
                                ...
                                <dd class="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
                                    <div class="mt-2">
                                        <input type="text" name="author" id="author" autocomplete="given-name" value="{{ old('author') }}" class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6">
                                        @error('author')
                                            <div class="text-red-600">{{ $message }}</div>
                                        @enderror
                                    </div>
                            ...
                            <div class="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
                                ...
                                <dd class="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
                                    <div class="mt-2">
                                        <input type="text" name="publication" id="publication" autocomplete="given-name" value="{{ old('publication') }}" class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6">
                                        @error('publication')
                                            <div class="text-red-600">{{ $message }}</div>
                                        @enderror
                                    </div>
                            ...
                            <div class="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
                                ...
                                <dd class="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
                                    <div class="mt-2">
                                        <textarea id="note" name="note" rows="6" class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6">{{ old('note') }}</textarea>
                                        @error('note')
                                            <div class="text-red-600">{{ $message }}</div>
                                        @enderror
                                    </div>
                    ...
                    <div class="flex justify-center">
                        ...
                        <button type="submit" class="mt-4 mr-2 shadow bg-gray-500 hover:bg-blue-500 focus:shadow-outline focus:outline-none text-white font-bold py-2 px-4 rounded">新規登録確定</button> 
                    </div>
                    {{-- 戻るボタンと変更ボタンの表示 --}}
                </form>
                </div>
